...
-- Delete duplicate super admin entries (keep the first one)
...
-- Delete duplicate admin entries with same email (keep the one with SUPER_ADMIN role if exists)
DELETE a1 FROM admins a1
INNER JOIN admins a2 
WHERE a1.id > a2.id 
...
-- Delete admin entries with same email as super admin (keep super admin)
DELETE a1 FROM admins a1
INNER JOIN admins a2 
WHERE a1.id != a2.id 
AND a1.email = a2.email
